...
public class StaffAttendance  implements Initializable {
    ...
    private void loadList()
    {
        try {

            {
                ...
                ResultSet rs = connection.get_students_attend(sr.getInt("id"));
                while(rs.next())
                {
                   // System.out.println(rs.getString("student_id")+" "+rs.getString("student_class_section.student_name"));
                   if(!connection.checkStudentAttendance(rs.getInt("student_id"), today.format(a), "student_attendance")) {
                       student_atd_coll.add(new attend_student(rs.getInt("student_id"), rs.getString("student_name")
  ...
  @FXML
   ...
   for(int i=0;i<student_atd_coll.size();i++)
   {
     ...
     //  boolean p=  student_atd_coll.get(i).getPresent().selectedProperty().get();
...
//     {
//         connection.insert_student_attendance(teach,student_atd_coll.get(i).getStudent_id(),
//                 student_atd_coll.get(i).getName(),student_atd_coll.get(i).getDate(),student_atd_coll.get(i).getDay(),"absent",
//                 student_atd_coll.get(i).getExcuse().getText());
//     }
//     else {
//         connection.insert_student_attendance(teach,student_atd_coll.get(i).getStudent_id(),
//                 student_atd_coll.get(i).getName(),student_atd_coll.get(i).getDate(),student_atd_coll.get(i).getDay(),"present",
//                 student_atd_coll.get(i).getExcuse().getText());
//
//     }
   }
